...
#define PKT_CONTROL        0x04  // 100：控制请求（Data=CtrlHead(+可选ID) / CtrlHead+ParameterList）
#define PKT_CONTROL_ACK    0x05  // 101：控制响应（Data=见各分支：CommandStatus 或 ID 回显 或 ParameterList）
...
// 规范对应：
//   - dtype  ↔ DataType(14b)
//   - lflag  ↔ LengthFlag(2b) 语义：00=无length固定4B；01=length 1B；10=length 2B；11=length 3B
//   - len    ↔ length(xB)      （当 lflag==00 时，本字段为解析出的数据长度 4）
//   - val    ↔ Data(lenB)
// ⚠ 不改变量名，仅用注释标注其语义。
typedef struct {
    uint16_t dtype;   // 规范：DataType(14b)。存放在低14位，发送时 (dtype<<2)|lflag → TypeLen(2B,LE)
    uint8_t  lflag;   // 规范：LengthFlag(2b)。00:无length=固定4B；01:len1B；10:len2B；11:len3B
    uint32_t len;     // 规范：length。lflag==00 时，解析器赋值为 4；其余为读取到的长度（LE）
